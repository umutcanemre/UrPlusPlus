...
struct Colour {
  char r, g, b, a;
  operator SDL_Color() {
    return SDL_Color{static_cast<unsigned char>(r),
...
class Screen {
 ...
 public:
  ...
  // Draw a string, requires a font to be loaded. By default font "COMICSANS" is already loaded.
  // You can add additional ones by downloading True Type Font files and loading them with the
  // add_font function.
  void draw_string(std::string msg, int x, int y, Colour c, std::string fontKey="COMICSANS");
