...
vector<pair<float, pair<size_t, size_t>>> Level3AI::assignPriorities( 
    const vector<pair<size_t, size_t>> &movelist, const GameState &gameState) {
    // Level 3: an AI that favours certain beneficial moves over others.
...
// level 3 AI visitor implementations - knows how to check tiles
void Level3AI::visitTileTornado(const TileTornado& t) { setTileScore(-1 + valueOfOccupant(t)); }
void Level3AI::visitTileBlackHole(const TileBlackHole& t) { setTileScore(-2 + valueOfOccupant(t)); }
void Level3AI::visitTileLucky(const TileLucky& t) { setTileScore(1 + valueOfOccupant(t)); }
void Level3AI::visitTileRosette(const TileRosette& t) { setTileScore(3 + valueOfOccupant(t)); }
void Level3AI::visitTileBasic(const TileBasic& t) { setTileScore(0 + valueOfOccupant(t)); }
void Level3AI::visitTileNull(const TileNull& t) { setTileScore(0); }

int Level3AI::valueOfOccupant(const Tile& t) {
